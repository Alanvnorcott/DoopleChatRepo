...
import { createQueues, enqueueClient, removeFromQueues, tryMatch, } from "./matchmaking.js";
...
const rateLimiterConfig = { windowMs: 60_000, max: 30 };
const ipToWindow = new Map();
...
const sessionIdToClient = new Map();
const queues = createQueues();
const reportsQueue = [];
function isValidUuidV4(candidate) {
...
function getIp(req) {
    ...
    const raw = req.headers["x-forwarded-for"]?.split(",")[0];
    ...
    ws.on("message", (data) => {
        ...
        switch (msg.type) {
            case "join": {
                ...
                const info = { sessionId, mode, interests, ws, ip };
