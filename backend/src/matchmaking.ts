...
import type { ChatMode, ClientInfo } from "./types.js";

export interface MatchmakingQueues {
  waiting_text: string[];
  waiting_video: string[];
  // interest -> Set of sessionIds waiting with that interest
  interestPoolsText: Map<string, Set<string>>;
  interestPoolsVideo: Map<string, Set<string>>;
}

export function createQueues(): MatchmakingQueues {
  return {
    waiting_text: [],
    waiting_video: [],
...
export function enqueueClient(
  queues: MatchmakingQueues,
  client: ClientInfo
): void {
  ...
  for (const interest of client.interests) {
    ...
    if (!key) continue;
    const set = pools.get(key) ?? new Set<string>();
...
export function removeFromQueues(
  ...
  for (const interest of client.interests) {
    ...
    if (set) {
      ...
      if (set.size === 0) pools.delete(key);
    }
  }
}
...
export function tryMatch(
  queues: MatchmakingQueues,
  client: ClientInfo,
  sessionIdToClient: Map<string, ClientInfo>
): MatchAttempt | null {
  const isVideo = client.mode === "video";
  const waiting = isVideo ? queues.waiting_video : queues.waiting_text;
  const pools = isVideo ? queues.interestPoolsVideo : queues.interestPoolsText;
  ...
  for (const interest of client.interests) {
    ...
    for (const sid of set) {
      if (sid !== client.sessionId) candidateSet.add(sid);
    }
  }
  ...
  if (candidateSet.size > 0) {
    for (const sid of waiting) {
      ...
      if (candidateSet.has(sid)) {
        ...
        if (!other || other.mode !== client.mode) continue;
