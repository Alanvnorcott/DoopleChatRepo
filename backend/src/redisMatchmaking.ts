 ...
 */
export async function connectRedis(cfg: RedisConfig): Promise<RedisMM> {
...
export function queueKey(mode: ChatMode) {
  return `queue:${mode}`;
}

export function poolKey(mode: ChatMode, interest: string) {
  return `pool:${mode}:${interest}`;
}

export function presenceKey(sessionId: string) {
  return `presence:${sessionId}`;
}

export function signalChannel(sessionId: string) {
...
export async function removeFromAll(
  mm: RedisMM,
  mode: ChatMode,
  sessionId: string,
  interests: string[]
...
export async function tryDequeuePair(
  mm: RedisMM,
  mode: ChatMode,
  sessionId: string,
  interests: string[]
